...
describe ShoppingCartController do
  ...
  describe '#checkout' do
    it 'calculates price correctly' do
      ...
      expect(assigns(:total_price)).to eq("0.00")
    end
  ...
  describe "#destroy_all" do
    it 'destroys all shopping cart and redirects to products_path' do
      product = Product.create(name: 'test', category: 'SomeCategory', quality: 'SomeQuality', is_sold: false, user_id: 1, product_traffic: 5)
      user = User.create(email: 'test@example.com', name: 'Test User', session_token: 'your_session_token')
      cart = ShoppingCart.create(product_id: product.id, user_id: user.id)
      allow(ShoppingCart).to receive(:destroy).and_return(cart.id)
      controller.instance_variable_set(:@current_user, user)
      delete :destroy_all
      expect(flash[:notice]).to eq("Shopping Cart Cleared!")
      expect(response).to redirect_to(view_shopping_cart_path)
    end
  end
end